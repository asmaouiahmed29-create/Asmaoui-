...
function genId() {
  return `FIN-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;
}
...
function pageShell(content: string, pageNum: number, total: number, title: string) {
  ...
  <div style="width:794px;min-height:1123px;background:${C.bg};font-family:'Cairo','Inter',sans-serif;
    ...
    <div style="border-top:1px solid ${C.border};padding:8px 36px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
      <span style="font-size:9px;color:${C.muted};">نظام OptimDZ للتحليل المالي — Système OptimDZ d'Analyse Financière CVP</span>
  ...
  <div style="width:794px;min-height:1123px;background:${C.primary};font-family:'Cairo','Inter',sans-serif;
    ...
    <div style="padding:28px 40px 0;display:flex;align-items:center;gap:12px;">
      ...
      <div>
        ...
        <div style="font-size:10px;color:rgba(255,255,255,0.65);margin-top:1px;">نظام دعم القرار · التحليل المالي CVP</div>
      </div>
    </div>
    <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:0 40px;text-align:center;gap:16px;">
      ...
      <div style="font-size:26px;font-weight:800;line-height:1.3;direction:rtl;">تقرير تحليل التعادل — CVP</div>
      <div style="font-size:16px;font-weight:400;color:rgba(255,255,255,0.8);">Rapport d'Analyse Coût-Volume-Profit</div>
      ...
      <div style="background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);border-radius:10px;padding:12px 28px;margin-top:8px;">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;text-align:center;">
          <div>
            <div style="font-size:9px;opacity:0.65;margin-bottom:3px;">السعر / Prix unitaire</div>
            <div style="font-size:16px;font-weight:800;">${fDA(result.input.sellingPrice)}</div>
          </div>
          <div>
            <div style="font-size:9px;opacity:0.65;margin-bottom:3px;">نقطة التعادل / Seuil</div>
            <div style="font-size:16px;font-weight:800;">${f(result.breakEvenUnits, 1)} unités</div>
          </div>
          <div>
            <div style="font-size:9px;opacity:0.65;margin-bottom:3px;">تكلفة التعادل / CA seuil</div>
            <div style="font-size:16px;font-weight:800;">${fDA(result.breakEvenRevenue)}</div>
          </div>
          <div>
            <div style="font-size:9px;opacity:0.65;margin-bottom:3px;">هامش المساهمة / Taux CM</div>
...
// ── Page 2 — CVP Analysis ─────────────────────────────────────────────────────
function buildCvpPage(result: BreakEvenResult, totalPages: number) {
  ...
  if (result.targetProfitUnits !== undefined) {
    optionalKpis.push(kpiCard(
      "وحدات لتحقيق الربح المستهدف · Unités pour bénéfice cible",
      `${f(result.targetProfitUnits, 1)} unités`,
      C.secondary
    ));
    optionalKpis.push(kpiCard(
      "CA pour bénéfice cible · رقم أعمال الهدف",
  ...
  // CVP table rows
  const tableRows = [
    ["Prix de vente unitaire", "السعر",                              fDA(inp.sellingPrice),         C.white],
    ["Coût variable unitaire", "التكلفة المتغيرة",                   fDA(inp.variableCost),          C.white],
    ["Marge / coût variable (CM)", "هامش المساهمة/وحدة",           fDA(cm),                        C.greenLight],
    ["Taux de marge (CM ratio)", "نسبة هامش المساهمة",              `${f(cmr, 2)} %`,               C.greenLight],
    ["Charges fixes totales", "الأعباء الثابتة الإجمالية",          fDA(inp.fixedCosts),            C.white],
    ["Seuil de rentabilité (unités)", "نقطة التعادل (وحدات)",        `${f(bepU, 2)} unités`,         C.orangeLight],
    ["Seuil de rentabilité (CA)", "نقطة التعادل (رقم الأعمال)",     fDA(bepR),                      C.orangeLight],
    ...(inp.expectedSalesVolume !== undefined ? [
      ["Volume de ventes prévu", "الحجم المتوقع",                    `${f(inp.expectedSalesVolume, 0)} unités`, C.white],
      ...
      ["Bénéfice net prévu", "الربح الصافي المتوقع",                  fDA(result.netProfit),          C.greenLight],
    ] : []),
    ...(inp.targetProfit !== undefined ? [
      ["Bénéfice cible", "الربح المستهدف",                           fDA(inp.targetProfit),          C.white],
  ...
  // Simple CVP graphical description box
  ...
  const content = `
    ${sectionTitle("Analyse Coût-Volume-Profit", "تحليل التعادل — CVP")}
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:18px;">
      ${kpiCard("نقطة التعادل (وحدات) · Seuil (unités)", f(bepU, 1) + " unités",          C.primary)}
      ${kpiCard("نقطة التعادل (CA) · Seuil (CA)",        fDA(bepR),                         C.orange)}
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:18px;">
      <div>
        ${subTitle("Tableau CVP Complet", "الجدول الشامل للتعادل", C.primary)}
      ...
      <div>
        ${subTitle("تمثيل بياني للتعادل · Représentation CVP", "Graphique CVP", C.orange)}
        <div style="background:${C.white};border:1px solid ${C.border};border-radius:8px;padding:14px;font-size:10px;">
          <div style="margin-bottom:8px;font-weight:700;font-size:11px;color:${C.primary};">نقطة التعادل على المحور · Position sur l'axe des unités</div>
          ...
          <div style="display:flex;justify-content:space-between;color:${C.muted};margin-bottom:12px;">
            ...
            ${esvPct ? `<span style="color:${C.secondary};font-weight:700;">● Vol: ${f(inp.expectedSalesVolume, 0)}</span>` : ""}
          ...
          <div style="font-size:10px;color:${C.text};line-height:1.6;">
            ...
            <div style="margin-bottom:4px;"><strong>Point d'intersection:</strong> (${f(bepU, 1)} unités ; ${fDA(bepR)})</div>
        ...
        <div style="margin-top:12px;background:${C.primaryLight};border-radius:8px;padding:12px;">
          <div style="font-size:10px;font-weight:700;color:${C.primary};margin-bottom:6px;">صيغ CVP المستخدمة · Formules CVP</div>
          <div style="font-family:monospace;color:${C.text};line-height:1.9;font-size:10px;">
            ...
            Seuil (Q) = CF ÷ CM/u = ${f(bepU, 2)} unités<br/>
            Seuil (CA) = CF ÷ Taux CM = ${fDA(bepR)}
            ${inp.expectedSalesVolume ? `<br/>MS% = (Q−Q*) ÷ Q × 100 = ${f(result.marginOfSafetyPct, 2)} %` : ""}
            ${result.operatingLeverage ? `<br/>DOL = CM totale ÷ Résultat = ×${f(result.operatingLeverage, 2)}` : ""}
          </div>
        </div>
      </div>
    </div>`;
  return pageShell(content, 2, totalPages, "تحليل CVP · Analyse Coût-Volume-Profit");
}

// ── Page 3 — Situational Analysis & Recommendations ──────────────────────────
function buildAnalysisPage(
  result: BreakEvenResult, sector: string | undefined, totalPages: number
) {
  const { input: inp, contributionMarginRatio: cmr, breakEvenUnits: bepU } = result;

  // Situational analysis
  const riskLevel  = cmr < 30 ? "élevé" : cmr < 50 ? "modéré" : "faible";
  const riskLevelAr = cmr < 30 ? "عالية" : cmr < 50 ? "متوسطة" : "منخفضة";

  const analysisItems: [string, string, string, string][] = [
    ["📊", `نشاط "${inp.productName || "—"}" ينتمي إلى قطاع ${sectorLabel(sector)}. سعر البيع ${inp.sellingPrice.toLocaleString("fr-DZ")} DA مقابل تكلفة متغيرة ${inp.variableCost.toLocaleString("fr-DZ")} DA وأعباء ثابتة ${inp.fixedCosts.toLocaleString("fr-DZ")} DA.`,
     C.primaryLight, C.primary + "50"],
    ["📈", `نقطة التعادل تقع عند ${f(bepU, 1)} وحدة (رقم أعمال ${inp.fixedCosts > 0 ? Math.round(result.breakEvenRevenue).toLocaleString("fr-DZ") : "0"} DA). هامش المساهمة لكل وحدة ${Math.round(result.contributionMarginPerUnit).toLocaleString("fr-DZ")} DA، ونسبة الهامش ${f(cmr, 1)}%.`,
     C.greenLight, C.green + "50"],
    [`${cmr >= 50 ? "✅" : cmr >= 30 ? "⚠️" : "🔴"}`,
     `مستوى المخاطرة التشغيلية ${riskLevelAr}. نسبة هامش المساهمة ${f(cmr, 1)}% — ${cmr >= 50 ? "هامش مريح، هيكل تكاليف صحي." : cmr >= 30 ? "هامش مقبول، مراجعة الأسعار أو التكاليف المتغيرة مستحسنة." : "هامش ضعيف، الحجم الكبير ضروري للربحية."}`,
    ...
    ...(result.marginOfSafetyPct !== undefined ? [[
      `${result.marginOfSafetyPct >= 20 ? "🛡️" : "⚠️"}`,
      `هامش الأمان ${f(result.marginOfSafetyPct, 1)}% (${f(result.marginOfSafetyUnits, 1)} وحدة، ${Math.round(result.marginOfSafetyRevenue ?? 0).toLocaleString("fr-DZ")} DA). ${result.marginOfSafetyPct >= 25 ? "منطقة آمنة — المبيعات يمكن أن تتراجع بهذه النسبة قبل الوصول للخسارة." : result.marginOfSafetyPct >= 10 ? "هامش ضيق — يستوجب مراقبة دقيقة للحجم الفعلي." : "هامش حرج — المؤسسة قريبة جداً من نقطة التعادل."}`,
  ...
  const suggestions: [string, string, string, string, string][] = [
    ["⚠️", "مراقبة نقطة التعادل · Surveiller le seuil", `الأولوية القصوى للوصول إلى الحد الأدنى ${f(bepU, 1)} وحدة شهرياً. أي نقصان في المبيعات يُولّد خسائر. يُستحسن مراجعة حجم الإنتاج الفعلي أسبوعياً.`, C.primary, C.primaryLight],
    ...(cmr < 40 ? [["💰", "تحسين نسبة الهامش · Améliorer le taux CM",
      `نسبة هامش المساهمة ${f(cmr, 1)}% منخفضة. مراجعة سعر البيع أو التفاوض على تخفيض التكاليف المتغيرة سيرفع الهامش ويخفض نقطة التعادل.`,
    ...
    ...(result.marginOfSafetyPct !== undefined && result.marginOfSafetyPct < 20 ? [[
      "🔴", "هامش أمان حرج · Marge de sécurité critique",
      `هامش الأمان ${f(result.marginOfSafetyPct, 1)}% أقل من 20% — حد الخطر. استراتيجية تنويع قنوات البيع أو خفض الأعباء الثابتة ضرورية لتوسيع المنطقة الآمنة.`,
      C.orange, C.orangeLight] as [string, string, string, string, string]] : []),
    ...(inp.fixedCosts / (inp.sellingPrice * bepU) > 0.5 ? [[
      "🏭", "أعباء ثابتة مرتفعة · Charges fixes élevées",
      `الأعباء الثابتة تُمثّل ${f((inp.fixedCosts / (inp.sellingPrice * bepU)) * 100, 1)}% من رقم أعمال التعادل — هيكل تكاليف صلب. يُقترح تحليل كل عبء ثابت وإمكانية تحويله إلى عبء متغير (مثل الاستعانة بعمالة مؤقتة بدل دائمة).`,
      C.secondary, C.greenLight] as [string, string, string, string, string]] : []),
    ["📊", "تطوير خطة المبيعات · Développer le plan commercial",
      `استناداً إلى نقطة التعادل ${f(bepU, 1)} وحدة، ضع هدفاً شهرياً بـ ${f(bepU * 1.3, 1)} وحدة (+30%) لضمان ربح ثابت ومتنامٍ. حدّد المجزوءات المربحة وأعِد توجيه الجهود التسويقية نحوها.`,
  ...
  const content = `
    ${sectionTitle("Analyse Situationnelle", "التحليل الموقفي")}
    ${analysisHtml}
    <div style="margin-top:20px;">
      ${sectionTitle("Recommandations Managériales", "التوصيات الإدارية")}
      ${suggestionsHtml}
    </div>`;
...
function buildStampPage(reportId: string, generatedAt: string, totalPages: number) {
  const content = `
    <div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:20px;text-align:center;">
      ...
      <div>
        <div style="font-size:22px;font-weight:800;color:${C.primary};">تقرير معتمد · Rapport Certifié</div>
        <div style="font-size:13px;color:${C.muted};margin-top:4px;">نظام OptimDZ للتحليل المالي CVP</div>
      </div>
      ...
      <div style="font-size:11px;color:${C.muted};max-width:420px;line-height:1.7;">
        هذا التقرير صادر تلقائياً من نظام OptimDZ لتحليل التعادل CVP. 
        القيم المحسوبة مبنية على البيانات المُدخلة وتُعدّ مؤشرات مساعدة للقرار.
        <br/><br/>
        Ce rapport a été généré automatiquement par le système OptimDZ d'Analyse CVP.
        Les valeurs calculées sont basées sur les données saisies et constituent des indicateurs d'aide à la décision.
      </div>
    </div>`;
...
export async function generateFinancialPDFReport(opts: FinancialPDFOptions): Promise<void> {
  const { result, sector, managerName = "", institutionName = "" } = opts;
  ...
  // Page 1 cover — a4 landscape-like
  const coverHtml = buildCover(result, sector, managerName, institutionName, reportId, generatedAt, totalPages);
  const cvpHtml   = buildCvpPage(result, totalPages);
  const analysisHtml = buildAnalysisPage(result, sector, totalPages);
  const stampHtml = buildStampPage(reportId, generatedAt, totalPages);

  // Render cover first (special handling — covers are 794px wide)
  ...
  await addHtmlPage(pdf, cvpHtml,      PAGE_W);
  await addHtmlPage(pdf, analysisHtml, PAGE_W);
  await addHtmlPage(pdf, stampHtml,    PAGE_W);

  const filename = `OptimDZ_CVP_${(result.input.productName || "rapport").replace(/\s+/g, "_")}_${Date.now()}.pdf`;
  pdf.save(filename);
}
